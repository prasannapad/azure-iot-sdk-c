...
AMQP_DEVICE_HANDLE iothubtransport_amqp_device_create(DEVICE_CONFIG *config)
{
    AMQP_DEVICE_INSTANCE *instance;

    // Codes_SRS_DEVICE_09_001: [If config, authorization_module or iothub_host_fqdn or on_state_changed_callback are NULL then iothubtransport_amqp_device_create shall fail and return NULL]
    if (config == NULL)
    {
    ...
    // Codes_SRS_DEVICE_09_002: [iothubtransport_amqp_device_create shall allocate memory for the device instance structure]
    else if ((instance = (AMQP_DEVICE_INSTANCE*)malloc(sizeof(AMQP_DEVICE_INSTANCE))) == NULL)
    {
        // Codes_SRS_DEVICE_09_003: [If malloc fails, iothubtransport_amqp_device_create shall fail and return NULL]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_005: [If any `config` parameters fail to be saved into `instance`, iothubtransport_amqp_device_create shall fail and return NULL]
        ...
        {
            // Codes_SRS_DEVICE_09_007: [If the AUTHENTICATION_HANDLE fails to be created, iothubtransport_amqp_device_create shall fail and return NULL]
        ...
        {
            // Codes_SRS_DEVICE_09_009: [If the TELEMETRY_MESSENGER_HANDLE fails to be created, iothubtransport_amqp_device_create shall fail and return NULL]
        ...
        {
            // Codes_SRS_DEVICE_09_123: [If the TWIN_MESSENGER_HANDLE fails to be created, iothubtransport_amqp_device_create shall fail and return NULL]
        ...
        if (result != RESULT_OK)
        {
            // Codes_SRS_DEVICE_09_010: [If iothubtransport_amqp_device_create fails it shall release all memory it has allocated]
            internal_destroy_device(instance);
            instance = NULL;
        }
    }

    // Codes_SRS_DEVICE_09_011: [If iothubtransport_amqp_device_create succeeds it shall return a handle to its `instance` structure]
    return (AMQP_DEVICE_HANDLE)instance;
}

int iothubtransport_amqp_device_start_async(AMQP_DEVICE_HANDLE handle, SESSION_HANDLE session_handle, CBS_HANDLE cbs_handle)
{
    int result;

    // Codes_SRS_DEVICE_09_017: [If `handle` is NULL, iothubtransport_amqp_device_start_async shall return a non-zero result]
    if (handle == NULL)
    {
    ...
    else
    {
        ...
        // Codes_SRS_DEVICE_09_018: [If the device state is not DEVICE_STATE_STOPPED, iothubtransport_amqp_device_start_async shall return a non-zero result]
        if (instance->state != DEVICE_STATE_STOPPED)
        {
            LogError("Failed starting device (device is not stopped)");
            result = __FAILURE__;
        }
        // Codes_SRS_DEVICE_09_019: [If `session_handle` is NULL, iothubtransport_amqp_device_start_async shall return a non-zero result]
        else if (session_handle == NULL)
        {
            LogError("Failed starting device (session_handle is NULL)");
            result = __FAILURE__;
        }
        // Codes_SRS_DEVICE_09_020: [If using CBS authentication and `cbs_handle` is NULL, iothubtransport_amqp_device_start_async shall return a non-zero result]
        ...
        else
        {
            ...
            // Codes_SRS_DEVICE_09_023: [If no failures occur, iothubtransport_amqp_device_start_async shall return 0]
            result = RESULT_OK;
        }
    }
...
int iothubtransport_amqp_device_stop(AMQP_DEVICE_HANDLE handle)
{
    int result;

    // Codes_SRS_DEVICE_09_024: [If `handle` is NULL, iothubtransport_amqp_device_stop shall return a non-zero result]
    if (handle == NULL)
    {
    ...
    else
    {
        ...
        // Codes_SRS_DEVICE_09_025: [If the device state is already DEVICE_STATE_STOPPED or DEVICE_STATE_STOPPING, iothubtransport_amqp_device_stop shall return a non-zero result]
        ...
        else
        {
            ...
            else
            {
                ...
                // Codes_SRS_DEVICE_09_032: [If no failures occur, iothubtransport_amqp_device_stop shall return 0]
                result = RESULT_OK;
            }
        }
    }
...
void iothubtransport_amqp_device_do_work(AMQP_DEVICE_HANDLE handle)
{
    // Codes_SRS_DEVICE_09_033: [If `handle` is NULL, iothubtransport_amqp_device_do_work shall return]
    if (handle == NULL)
    {
        LogError("Failed to perform iothubtransport_amqp_device_do_work (handle is NULL)");
    }
    else
    {
...
void iothubtransport_amqp_device_destroy(AMQP_DEVICE_HANDLE handle)
{
    // Codes_SRS_DEVICE_09_012: [If `handle` is NULL, iothubtransport_amqp_device_destroy shall return]
    if (handle == NULL)
    {
    ...
    {
        AMQP_DEVICE_INSTANCE* instance = (AMQP_DEVICE_INSTANCE*)handle;
        // Codes_SRS_DEVICE_09_013: [If the device is in state DEVICE_STATE_STARTED or DEVICE_STATE_STARTING, iothubtransport_amqp_device_stop() shall be invoked]
        if (instance->state == DEVICE_STATE_STARTED || instance->state == DEVICE_STATE_STARTING)
        {
            (void)iothubtransport_amqp_device_stop((AMQP_DEVICE_HANDLE)instance);
        }
...
int iothubtransport_amqp_device_send_event_async(AMQP_DEVICE_HANDLE handle, IOTHUB_MESSAGE_LIST* message, ON_DEVICE_D2C_EVENT_SEND_COMPLETE on_device_d2c_event_send_complete_callback, void* context)
{
    int result;

    // Codes_SRS_DEVICE_09_051: [If `handle` are `message` are NULL, iothubtransport_amqp_device_send_event_async shall return a non-zero result]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_053: [If `send_task` fails to be created, iothubtransport_amqp_device_send_event_async shall return a non-zero value]
        ...
        else
        {
            ...
            {
                // Codes_SRS_DEVICE_09_056: [If telemetry_messenger_send_async fails, iothubtransport_amqp_device_send_event_async shall return a non-zero value]
                LogError("Failed sending event (telemetry_messenger_send_async failed)");
                // Codes_SRS_DEVICE_09_057: [If any failures occur, iothubtransport_amqp_device_send_event_async shall release all memory it has allocated]
                free(send_task);
                result = __FAILURE__;
            }
            else
            {
                // Codes_SRS_DEVICE_09_058: [If no failures occur, iothubtransport_amqp_device_send_event_async shall return 0]
                result = RESULT_OK;
            }
        }
    }
...
int iothubtransport_amqp_device_get_send_status(AMQP_DEVICE_HANDLE handle, IOTHUBTRANSPORT_AMQP_DEVICE_SEND_STATUS *send_status)
{
    ...
    // Codes_SRS_DEVICE_09_105: [If `handle` or `send_status` is NULL, iothubtransport_amqp_device_get_send_status shall return a non-zero result]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_107: [If telemetry_messenger_get_send_status fails, iothubtransport_amqp_device_get_send_status shall return a non-zero result]
            LogError("Failed getting the device messenger send status (telemetry_messenger_get_send_status failed)");
            result = __FAILURE__;
        }
        else
        {
            // Codes_SRS_DEVICE_09_108: [If telemetry_messenger_get_send_status returns TELEMETRY_MESSENGER_SEND_STATUS_IDLE, iothubtransport_amqp_device_get_send_status return status DEVICE_SEND_STATUS_IDLE]
            if (messenger_send_status == TELEMETRY_MESSENGER_SEND_STATUS_IDLE)
            {
                *send_status = DEVICE_SEND_STATUS_IDLE;
            }
            // Codes_SRS_DEVICE_09_109: [If telemetry_messenger_get_send_status returns TELEMETRY_MESSENGER_SEND_STATUS_BUSY, iothubtransport_amqp_device_get_send_status return status DEVICE_SEND_STATUS_BUSY]
            else // i.e., messenger_send_status == TELEMETRY_MESSENGER_SEND_STATUS_BUSY
            {
                *send_status = DEVICE_SEND_STATUS_BUSY;
            }

            // Codes_SRS_DEVICE_09_110: [If iothubtransport_amqp_device_get_send_status succeeds, it shall return zero as result]
            result = RESULT_OK;
        }
    }
...
int iothubtransport_amqp_device_subscribe_message(AMQP_DEVICE_HANDLE handle, ON_DEVICE_C2D_MESSAGE_RECEIVED on_message_received_callback, void* context)
{
    int result;

    // Codes_SRS_DEVICE_09_066: [If `handle` or `on_message_received_callback` or `context` is NULL, iothubtransport_amqp_device_subscribe_message shall return a non-zero result]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_068: [If telemetry_messenger_subscribe_for_messages fails, iothubtransport_amqp_device_subscribe_message shall return a non-zero result]
        ...
        else
        {
            ...
            // Codes_SRS_DEVICE_09_069: [If no failures occur, iothubtransport_amqp_device_subscribe_message shall return 0]
            result = RESULT_OK;
        }
    }
...
int iothubtransport_amqp_device_unsubscribe_message(AMQP_DEVICE_HANDLE handle)
{
    int result;

    // Codes_SRS_DEVICE_09_076: [If `handle` is NULL, iothubtransport_amqp_device_unsubscribe_message shall return a non-zero result]
    if (handle == NULL)
    {
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_078: [If telemetry_messenger_unsubscribe_for_messages fails, iothubtransport_amqp_device_unsubscribe_message shall return a non-zero result]
            LogError("Failed unsubscribing to C2D messages (telemetry_messenger_unsubscribe_for_messages failed)");
            result = __FAILURE__;
        }
        else
        {
            // Codes_SRS_DEVICE_09_079: [If no failures occur, iothubtransport_amqp_device_unsubscribe_message shall return 0]
            result = RESULT_OK;
        }
    }
    return result;
...
int iothubtransport_amqp_device_send_message_disposition(AMQP_DEVICE_HANDLE device_handle, DEVICE_MESSAGE_DISPOSITION_INFO* disposition_info, IOTHUBTRANSPORT_AMQP_DEVICE_MESSAGE_DISPOSITION_RESULT disposition_result)
{
    ...
    // Codes_SRS_DEVICE_09_111: [If `device_handle` or `disposition_info` are NULL, iothubtransport_amqp_device_send_message_disposition() shall fail and return __FAILURE__]
    ...
    // Codes_SRS_DEVICE_09_112: [If `disposition_info->source` is NULL, iothubtransport_amqp_device_send_message_disposition() shall fail and return __FAILURE__]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_114: [If the TELEMETRY_MESSENGER_MESSAGE_DISPOSITION_INFO fails to be created, iothubtransport_amqp_device_send_message_disposition() shall fail and return __FAILURE__]
        ...
        else
        {
            ...
            {
                // Codes_SRS_DEVICE_09_116: [If `telemetry_messenger_send_message_disposition()` fails, iothubtransport_amqp_device_send_message_disposition() shall fail and return __FAILURE__]
                LogError("Failed sending message disposition (telemetry_messenger_send_message_disposition failed)");
                result = __FAILURE__;
            }
            else
            {
                // Codes_SRS_DEVICE_09_118: [If no failures occurr, iothubtransport_amqp_device_send_message_disposition() shall return 0]
                result = RESULT_OK;
            }

            // Codes_SRS_DEVICE_09_117: [iothubtransport_amqp_device_send_message_disposition() shall destroy the TELEMETRY_MESSENGER_MESSAGE_DISPOSITION_INFO instance]
...
int iothubtransport_amqp_device_set_retry_policy(AMQP_DEVICE_HANDLE handle, IOTHUB_CLIENT_RETRY_POLICY policy, size_t retry_timeout_limit_in_seconds)
{
    (void)retry_timeout_limit_in_seconds;
    (void)policy;
    int result;

    // Codes_SRS_DEVICE_09_080: [If `handle` is NULL, iothubtransport_amqp_device_set_retry_policy shall return a non-zero result]
    if (handle == NULL)
    {
    ...
    {
        // Codes_SRS_DEVICE_09_081: [iothubtransport_amqp_device_set_retry_policy shall return a non-zero result]
...
int iothubtransport_amqp_device_set_option(AMQP_DEVICE_HANDLE handle, const char* name, void* value)
{
    int result;

    // Codes_SRS_DEVICE_09_082: [If `handle` or `name` or `value` are NULL, iothubtransport_amqp_device_set_option shall return a non-zero result]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_083: [If `name` refers to authentication but CBS authentication is not used, iothubtransport_amqp_device_set_option shall return a non-zero result]
            if (instance->authentication_handle == NULL)
            {
            ...
            {
                // Codes_SRS_DEVICE_09_085: [If authentication_set_option fails, iothubtransport_amqp_device_set_option shall return a non-zero result]
        ...
        else if (strcmp(DEVICE_OPTION_EVENT_SEND_TIMEOUT_SECS, name) == 0)
        {
            ...
            {
                // Codes_SRS_DEVICE_09_087: [If telemetry_messenger_set_option fails, iothubtransport_amqp_device_set_option shall return a non-zero result]
        ...
        {
            // Codes_SRS_DEVICE_09_088: [If `name` is DEVICE_OPTION_SAVED_AUTH_OPTIONS but CBS authentication is not being used, iothubtransport_amqp_device_set_option shall return a non-zero result]
            if (instance->authentication_handle == NULL)
            {
                LogError("failed setting option for device '%s' (cannot set authentication option '%s'; not using CBS authentication)", instance->config->device_id, name);
                result = __FAILURE__;
            ...
            {
                // Codes_SRS_DEVICE_09_091: [If any call to OptionHandler_FeedOptions fails, iothubtransport_amqp_device_set_option shall return a non-zero result]
        ...
        {
            // Codes_SRS_DEVICE_09_092: [If no failures occur, iothubtransport_amqp_device_set_option shall return 0]
...
OPTIONHANDLER_HANDLE iothubtransport_amqp_device_retrieve_options(AMQP_DEVICE_HANDLE handle)
{
    OPTIONHANDLER_HANDLE result;

    // Codes_SRS_DEVICE_09_093: [If `handle` is NULL, iothubtransport_amqp_device_retrieve_options shall return NULL]
    if (handle == NULL)
    {
    ...
    else
    {
        ...
        OPTIONHANDLER_HANDLE options = OptionHandler_Create(device_clone_option, device_destroy_option, (pfSetOption)iothubtransport_amqp_device_set_option);

        if (options == NULL)
        {
            // Codes_SRS_DEVICE_09_095: [If OptionHandler_Create fails, iothubtransport_amqp_device_retrieve_options shall return NULL]
        ...
        else
        {
            ...
            {
                // Codes_SRS_DEVICE_09_097: [If authentication_retrieve_options fails, iothubtransport_amqp_device_retrieve_options shall return NULL]
            ...
            {
                // Codes_SRS_DEVICE_09_102: [If any call to OptionHandler_AddOption fails, iothubtransport_amqp_device_retrieve_options shall return NULL]
            ...
            {
                // Codes_SRS_DEVICE_09_100: [If telemetry_messenger_retrieve_options fails, iothubtransport_amqp_device_retrieve_options shall return NULL]
            ...
            if (result == NULL)
            {
                // Codes_SRS_DEVICE_09_103: [If any failure occurs, any memory allocated by iothubtransport_amqp_device_retrieve_options shall be destroyed]
...
int iothubtransport_amqp_device_send_twin_update_async(AMQP_DEVICE_HANDLE handle, CONSTBUFFER_HANDLE data, DEVICE_SEND_TWIN_UPDATE_COMPLETE_CALLBACK on_send_twin_update_complete_callback, void* context)
{
    int result;

    // Codes_SRS_DEVICE_09_135: [If `handle` or `data` are NULL, iothubtransport_amqp_device_send_twin_update_async shall return a non-zero result]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_137: [If `twin_ctx` fails to be created, iothubtransport_amqp_device_send_twin_update_async shall return a non-zero value]
        ...
        else
        {
            ...
            {
                // Codes_SRS_DEVICE_09_139: [If twin_messenger_report_state_async fails, iothubtransport_amqp_device_send_twin_update_async shall return a non-zero value]
            ...
            {
                // Codes_SRS_DEVICE_09_140: [If no failures occur, iothubtransport_amqp_device_send_twin_update_async shall return 0]
                result = RESULT_OK;
            }
        }
    }
...
int iothubtransport_amqp_device_subscribe_for_twin_updates(AMQP_DEVICE_HANDLE handle, DEVICE_TWIN_UPDATE_RECEIVED_CALLBACK on_device_twin_update_received_callback, void* context)
{
    int result;

    // Codes_SRS_DEVICE_09_143: [If `handle` or `on_device_twin_update_received_callback` are NULL, iothubtransport_amqp_device_subscribe_for_twin_updates shall return a non-zero result]
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_145: [If twin_messenger_subscribe fails, iothubtransport_amqp_device_subscribe_for_twin_updates shall return a non-zero value]
        ...
        {
            // Codes_SRS_DEVICE_09_146: [If no failures occur, iothubtransport_amqp_device_subscribe_for_twin_updates shall return 0]
            result = RESULT_OK;
        }
    }
...
int iothubtransport_amqp_device_unsubscribe_for_twin_updates(AMQP_DEVICE_HANDLE handle)
{
    int result;


    // Codes_SRS_DEVICE_09_147: [If `handle` is NULL, iothubtransport_amqp_device_unsubscribe_for_twin_updates shall return a non-zero result]
    if (handle == NULL)
    {
    ...
    else
    {
        ...
        {
            // Codes_SRS_DEVICE_09_149: [If twin_messenger_unsubscribe fails, iothubtransport_amqp_device_unsubscribe_for_twin_updates shall return a non-zero value]
        ...
        else
        {
            ...
            // Codes_SRS_DEVICE_09_150: [If no failures occur, iothubtransport_amqp_device_unsubscribe_for_twin_updates shall return 0]
            result = RESULT_OK;
        }
    }
...
int iothubtransport_amqp_device_get_twin_async(AMQP_DEVICE_HANDLE handle, DEVICE_TWIN_UPDATE_RECEIVED_CALLBACK on_device_get_twin_completed_callback, void* context)
{
    int result;

    // Codes_SRS_DEVICE_09_152: [If `handle` or `on_device_get_twin_completed_callback` are NULL, iothubtransport_amqp_device_get_twin_async shall return a non-zero result]
    ...
    else
    {
        ...
        else
        {
            ...
            {
                // Codes_SRS_DEVICE_09_154: [If twin_messenger_get_twin_async fails, iothubtransport_amqp_device_get_twin_async shall return a non-zero value]
            ...
            {
                // Codes_SRS_DEVICE_09_155: [If no failures occur, iothubtransport_amqp_device_get_twin_async shall return 0]
                result = RESULT_OK;
            }
        }
    }
