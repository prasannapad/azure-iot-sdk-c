...
static void internal_destroy_amqp_device_instance(AMQP_TRANSPORT_DEVICE_INSTANCE *trdev_inst)
{
    ...
    {
        iothubtransport_amqp_device_destroy(trdev_inst->device_handle);
    }
...
static void prepare_device_for_connection_retry(AMQP_TRANSPORT_DEVICE_INSTANCE* registered_device)
{
    ...
    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_031: [iothubtransport_amqp_device_stop() shall be invoked on all `instance->registered_devices` that are not already stopped]
    if (registered_device->device_state != DEVICE_STATE_STOPPED)
    {
        if (iothubtransport_amqp_device_stop(registered_device->device_handle) != RESULT_OK)
        {
            LogError("Failed preparing device '%s' for connection retry (iothubtransport_amqp_device_stop failed)", STRING_c_str(registered_device->device_id));
        }
    }
...
static int send_pending_events(AMQP_TRANSPORT_DEVICE_INSTANCE* device_state)
{
    ...
    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_047: [If the registered device is started, each event on `registered_device->wait_to_send_list` shall be removed from the list and sent using iothubtransport_amqp_device_send_event_async()]
    while ((message = get_next_event_to_send(device_state)) != NULL)
    {
        // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_048: [iothubtransport_amqp_device_send_event_async() shall be invoked passing `on_event_send_complete`]
        if (iothubtransport_amqp_device_send_event_async(device_state->device_handle, message, on_event_send_complete, device_state) != RESULT_OK)
        {
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_049: [If iothubtransport_amqp_device_send_event_async() fails, `on_event_send_complete` shall be invoked passing EVENT_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING and return]
            LogError("Device '%s' failed to send message (iothubtransport_amqp_device_send_event_async failed)", STRING_c_str(device_state->device_id));
            result = __FAILURE__;
...
static int IoTHubTransport_AMQP_Common_Device_DoWork(AMQP_TRANSPORT_DEVICE_INSTANCE* registered_device)
{
    ...
    if (registered_device->device_state != DEVICE_STATE_STARTED)
    {
        ...
        if (registered_device->device_state == DEVICE_STATE_STOPPED)
        {
            ...
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_041: [The device handle shall be started using iothubtransport_amqp_device_start_async()]
            else if (iothubtransport_amqp_device_start_async(registered_device->device_handle, session_handle, cbs_handle) != RESULT_OK)
            {
                // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_042: [If iothubtransport_amqp_device_start_async() fails, IoTHubTransport_AMQP_Common_DoWork shall fail and skip to the next registered device]
        ...
        else // i.e., DEVICE_STATE_ERROR_AUTH || DEVICE_STATE_ERROR_AUTH_TIMEOUT || DEVICE_STATE_ERROR_MSG
        {
            ...
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_045: [If the registered device has a failure, it shall be stopped using iothubtransport_amqp_device_stop()]
            else if (iothubtransport_amqp_device_stop(registered_device->device_handle) != RESULT_OK)
            {
                LogError("Failed to stop reset device '%s' (iothubtransport_amqp_device_stop failed)", STRING_c_str(registered_device->device_id));
                result = __FAILURE__;
            }
            else
            {
    ...
    iothubtransport_amqp_device_do_work(registered_device->device_handle);
...
static int replicate_device_options_to(AMQP_TRANSPORT_DEVICE_INSTANCE* dev_instance, DEVICE_AUTH_MODE auth_mode)
{
    ...
    if (iothubtransport_amqp_device_set_option(
    ...
    {
        LogError("Failed to apply option DEVICE_OPTION_EVENT_SEND_TIMEOUT_SECS to device '%s' (iothubtransport_amqp_device_set_option failed)", STRING_c_str(dev_instance->device_id));
        result = __FAILURE__;
    }
    else if (auth_mode == DEVICE_AUTH_MODE_CBS)
    {
        ...
        {
            LogError("Failed to apply option DEVICE_OPTION_CBS_REQUEST_TIMEOUT_SECS to device '%s' (iothubtransport_amqp_device_set_option failed)", STRING_c_str(dev_instance->device_id));
            result = __FAILURE__;
        }
        else
        {
...
static int IoTHubTransport_AMQP_Common_Device_SetOption(TRANSPORT_LL_HANDLE handle, const char* option, void* value)
{
    ...
    else
    {
        ...
        while (list_item != NULL)
        {
            ...
            else if (iothubtransport_amqp_device_set_option(registered_device->device_handle, device_option, value) != RESULT_OK)
            {
                LogError("failed setting option '%s' to registered device '%s' (iothubtransport_amqp_device_set_option failed)",
    ...
    else
    {
        if (item_type == IOTHUB_TYPE_DEVICE_TWIN)
        {
            ...
            else
            {
                ...
                // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_146: [iothubtransport_amqp_device_send_twin_update_async() shall be invoked passing `iothub_item->device_twin->report_data_handle` and `on_device_send_twin_update_complete_callback`]
                if (iothubtransport_amqp_device_send_twin_update_async(
                ...
                {
                    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_147: [If iothubtransport_amqp_device_send_twin_update_async() fails, `IoTHubTransport_AMQP_Common_ProcessItem` shall fail and return IOTHUB_PROCESS_ERROR.]
...
int IoTHubTransport_AMQP_Common_Subscribe(IOTHUB_DEVICE_HANDLE handle)
{
    ...
    else
    {
        ...
        // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_086: [iothubtransport_amqp_device_subscribe_message() shall be invoked passing `on_message_received_callback`]
        else if (iothubtransport_amqp_device_subscribe_message(amqp_device_instance->device_handle, on_message_received, amqp_device_instance) != RESULT_OK)
        {
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_087: [If iothubtransport_amqp_device_subscribe_message() fails, IoTHubTransport_AMQP_Common_Subscribe shall return a non-zero result]
            LogError("Device '%s' failed subscribing to cloud-to-device messages (iothubtransport_amqp_device_subscribe_message failed)", STRING_c_str(amqp_device_instance->device_id));
            result = __FAILURE__;
        }
        else
        {
...
void IoTHubTransport_AMQP_Common_Unsubscribe(IOTHUB_DEVICE_HANDLE handle)
{
    ...
    else
    {
        ...
        // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_095: [iothubtransport_amqp_device_unsubscribe_message() shall be invoked passing `amqp_device_instance->device_handle`]
        else if (iothubtransport_amqp_device_unsubscribe_message(amqp_device_instance->device_handle) != RESULT_OK)
        {
            LogError("Device '%s' failed unsubscribing to cloud-to-device messages (iothubtransport_amqp_device_unsubscribe_message failed)", STRING_c_str(amqp_device_instance->device_id));
        }
    }
}
    ...
    else
    {
        ...
        else
        {
            ...
            while (list_item != NULL)
            {
                ...
                // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_134: [iothubtransport_amqp_device_subscribe_for_twin_updates() shall be invoked for the registered device, passing `on_device_twin_update_received_callback`]
                else if (iothubtransport_amqp_device_subscribe_for_twin_updates(registered_device->device_handle, on_device_twin_update_received_callback, (void*)registered_device) != RESULT_OK)
                {
                    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_135: [If iothubtransport_amqp_device_subscribe_for_twin_updates() fails, `IoTHubTransport_AMQP_Common_Subscribe_DeviceTwin` shall fail and return non-zero.]
    ...
    else
    {
        ...
        else
        {
            ...
            while (list_item != NULL)
            {
                ...
                // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_142: [iothubtransport_amqp_device_unsubscribe_for_twin_updates() shall be invoked for the registered device]
                else if (iothubtransport_amqp_device_unsubscribe_for_twin_updates(registered_device->device_handle) != RESULT_OK)
                {
                    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_143: [If `iothubtransport_amqp_device_unsubscribe_for_twin_updates` fails, the error shall be ignored]
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_155: [ iothubtransport_amqp_device_get_twin_async() shall be invoked for the registered device, passing `on_device_get_twin_completed_callback`]
                if (iothubtransport_amqp_device_get_twin_async(registered_device->device_handle, on_device_get_twin_completed_callback, (void*)getTwinCtx) != RESULT_OK)
                {
                    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_156: [ If iothubtransport_amqp_device_get_twin_async() fails, `IoTHubTransport_AMQP_Common_GetTwinAsync` shall fail and return IOTHUB_CLIENT_ERROR ]
                    LogError("Failed subscribing for device Twin updates");
                    free(getTwinCtx);
                    result = IOTHUB_CLIENT_ERROR;
                }
...
IOTHUB_CLIENT_RESULT IoTHubTransport_AMQP_Common_GetSendStatus(IOTHUB_DEVICE_HANDLE handle, IOTHUB_CLIENT_STATUS *iotHubClientStatus)
{
    ...
    else
    {
        ...
        {
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_098: [If iothubtransport_amqp_device_get_send_status() fails, IoTHubTransport_AMQP_Common_GetSendStatus shall return IOTHUB_CLIENT_ERROR]
            LogError("Failed retrieving the device send status (iothubtransport_amqp_device_get_send_status failed)");
            result = IOTHUB_CLIENT_ERROR;
        }
        else
        {
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_099: [If iothubtransport_amqp_device_get_send_status() returns DEVICE_SEND_STATUS_BUSY, IoTHubTransport_AMQP_Common_GetSendStatus shall return IOTHUB_CLIENT_OK and status IOTHUB_CLIENT_SEND_STATUS_BUSY]
            if (device_send_status == DEVICE_SEND_STATUS_BUSY)
            {
                *iotHubClientStatus = IOTHUB_CLIENT_SEND_STATUS_BUSY;
            }
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_100: [If iothubtransport_amqp_device_get_send_status() returns DEVICE_SEND_STATUS_IDLE, IoTHubTransport_AMQP_Common_GetSendStatus shall return IOTHUB_CLIENT_OK and status IOTHUB_CLIENT_SEND_STATUS_IDLE]
...
IOTHUB_CLIENT_RESULT IoTHubTransport_AMQP_Common_SetOption(TRANSPORT_LL_HANDLE handle, const char* option, const void* value)
{
    ...
    else
    {
        ...
        if (is_device_specific_option)
        {
            ...
            {
                // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_103: [If iothubtransport_amqp_device_set_option() fails, IoTHubTransport_AMQP_Common_SetOption shall return IOTHUB_CLIENT_ERROR]
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_072: [The configuration for iothubtransport_amqp_device_create shall be set according to the authentication preferred by IOTHUB_DEVICE_CONFIG]
                    ...
                    // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_071: [`amqp_device_instance->device_handle` shall be set using iothubtransport_amqp_device_create()]
                    if ((amqp_device_instance->device_handle = iothubtransport_amqp_device_create(&device_config)) == NULL)
                    {
                        // Codes_SRS_IOTHUBTRANSPORT_AMQP_COMMON_09_073: [If iothubtransport_amqp_device_create() fails, IoTHubTransport_AMQP_Common_Register shall fail and return NULL]
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (iothubtransport_amqp_device_send_message_disposition(message_data->transportContext->device_state->device_handle, device_message_disposition_info, device_disposition_result) != RESULT_OK)
                {
                    LogError("Device '%s' failed sending message disposition (iothubtransport_amqp_device_send_message_disposition failed)", STRING_c_str(message_data->transportContext->device_state->device_id));
                    result = IOTHUB_CLIENT_ERROR;
                }
                else
                {
